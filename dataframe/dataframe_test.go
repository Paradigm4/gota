...
func TestDataFrame_Append(t *testing.T) {
	...
	for i, tc := range table {
		...
		// Check that the types are the same between both DataFrames
		if !reflect.DeepEqual(tc.expDf.Types(), b.Types()) {
			t.Errorf("Test: %d\nDifferent types:\nExpected:%v\nReceived:%v", i, tc.expDf.Types(), b.Types())
		}
		// Check that the colnames are the same between both DataFrames
		if !reflect.DeepEqual(tc.expDf.Names(), b.Names()) {
			t.Errorf("Test: %d\nDifferent colnames:\nExpected:%v\nReceived:%v", i, tc.expDf.Names(), b.Names())
		}
		// Check that the values are the same between both DataFrames
		expR, _ := tc.expDf.Records(true)
		recR, _ := b.Records(true)
		if !reflect.DeepEqual(expR, recR) {
			t.Errorf("Test: %d\nDifferent values:\nExpected:%v\nReceived:%v", i, expR, recR)
		}
	}
}
...
func TestReadHTML(t *testing.T) {
	...
	for i, tc := range table {
		...
		for j, c := range cs {
			...
			// Check that the values are the same between both DataFrames
			expR, _ := tc.expDf[j].Records(true)
			cr, _ := c.Records(true)
			if !reflect.DeepEqual(expR, cr) {
				t.Errorf("Test: %d\nDifferent values:\nA:%v\nB:%v", i, expR, cr)
			}
		}
	}
