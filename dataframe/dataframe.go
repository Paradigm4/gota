...
func (gps Groups) Aggregation(typs []AggregationType, colnames []string) DataFrame {
	...
	for _, df := range gps.groups {
		...
		for i, c := range colnames {
			...
			switch typs[i] {
			case Aggregation_MAX:
				value, _ = curSeries.Max()
			case Aggregation_MEAN:
				value, _ = curSeries.Mean()
			case Aggregation_MEDIAN:
				value, _ = curSeries.Median()
			case Aggregation_MIN:
				value, _ = curSeries.Min()
			case Aggregation_STD:
				value, _ = curSeries.StdDev()
			case Aggregation_SUM:
				value, _ = curSeries.Sum(true)
			case Aggregation_COUNT:
...
func (df DataFrame) Append(dfb DataFrame, force bool) DataFrame {
	...
	for k, v := range cols {
		...
		if a.Type() != b.Type() && !force {
			df.Err = fmt.Errorf("Append: '%s' columns have different types: %s vs %s", v, a.Type(), b.Type())
			return df
		}
		a.Append(b)
		if err := a.Err; err != nil {
			return DataFrame{Err: fmt.Errorf("append: %v", err)}
		}
		expandedSeries[k] = a
	}
